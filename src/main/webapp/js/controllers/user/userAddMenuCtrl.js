/**
 * 用户中心-菜单管理
 * 现在这个菜单用的是以前的表格，目前将用户-菜单，砍掉了
 * 后面加上来，可能需要把表格换成菜单，参照roleCtrl2.js
*/
angular.module('inspinia').controller('userAddMenuCtrl',function($scope,$http,$state,$stateParams,i18nService,$timeout){
...
//            {field: 'orderNo',displayName: '排序'}
	...
	$scope.baseInfo.userName = $stateParams.userName;
	//查询
	$scope.query = function(){
		$http.post('user/selectMenuByUser.do',"baseInfo="+angular.toJson($scope.baseInfo)+"&pageNo="+$scope.paginationOptions.pageNo+"&pageSize="+
	...
	$scope.editMenu = function(){
		...
		var param = {"userId":$stateParams.id,"menuIds":menuIds};
		$http.post("user/editUserMenu.do",angular.toJson(param))
			.success(function(msg){
				$scope.notice(msg.msg);
				$state.transitionTo('user.user',null,{reload:true});
